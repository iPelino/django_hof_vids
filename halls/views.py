...
YOUTUBE_API_KEY = 'AIzaSyDHYyX8cUL1P35PZeSEBEha2IQtQ9tgDa4'
...
def add_video(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            video_id = url_query.replace('v=', '')
            if video_id:
                video.youtube_id = video_id
                response = requests.get(f'https://www.googleapis.com/youtube/v3/videos?part=snippet&id={video_id}&key={YOUTUBE_API_KEY}')
                json = response.json()
...
def video_search(request):
    ...
    if search_form.is_valid():
        ...
        print(search_term)
        response = requests.get(f'https://www.googleapis.com/youtube/v3/search?part=snippet&maxResults=5&q={ search_term }&key={YOUTUBE_API_KEY}')
...
class DeleteVideo(LoginRequiredMixin, generic.DeleteView):
    ...
    # Ensure that you delete only videos you created
    def get_object(self):
